...
def add_seam(img1,rows,cols1,jj):
 img2 = np.zeros((rows,cols1+1,3), np.uint8)
 for i in range(rows):
  jadd = jj[i]
  img2[i,0:jadd+1,:] = img1[i,0:jadd+1,:]
  img2[i,jadd+1,:] = img1[i,jadd,:]
  img2[i,jadd+2:cols1+1,:] = img1[i,jadd+1:cols1,:]   
 return img2

def vertical_seam_enlarge(cols_desired,img):
 ...
 while cols1 < cols_desired:
  ...
  img1 = add_seam(img1,rows,cols1,jj)
  cols1 += 1  
 return img1 
...
if __name__ == '__main__':
 ...
 cols_desired = cols + 50
 img_out = vertical_seam_enlarge(cols_desired,img)
 print "dimensions of seam carved image = ", img_out.shape[:2]

 cv2.imshow('original image', img)
 cv2.imshow('seam carved image',img_out)
 cv2.imwrite('seam_carved_enlarged.jpg',img_out)
 cv2.waitKey()
